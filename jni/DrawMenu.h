...
inline bool RefreshEnemyRoomInfo(std::vector<RoomEnemyInfoRow> &outRows) {
    ...
    if (roomPlayers->getSize() > 20) return false;
...
inline bool ShouldRefreshRoomInfoNow() {
    if (IsInBattleMatchNow()) return false;

    const int battleState = GetBattleState();
    if (battleState != 2 && battleState != 3) return false;
    ...
    if (battleManagerInstance) {
        const uintptr_t localPlayerOffset = BattleManager_m_LocalPlayerShow();
	...
	if (!isLogin) {
		...
		if (ImGui::BeginTabBar("Tab", ImGuiTabBarFlags_FittingPolicyScroll)) {
            ...
            if (ImGui::BeginTabItem("Room Info")) {
                ...
                const bool allowRefresh = ShouldRefreshRoomInfoNow();
                const bool shouldAutoRefresh = (frameNow - lastRefreshFrame) >= 30;
                if (allowRefresh && (!hasSnapshot || shouldAutoRefresh)) {
                ...
                if (!allowRefresh) {
                    ImGui::TextColored(RGBA2ImVec4(255, 200, 0, 255), "Room info refresh is paused outside matchmaking.");
